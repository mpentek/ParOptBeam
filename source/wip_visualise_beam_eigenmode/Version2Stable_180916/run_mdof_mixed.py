...
# Create object of the MDoFBeamModel
beam_model = MDoFBeamModel(rho, area, target_freq, target_mode, zeta, floor_height, floor_num, name)

# k stiffness value for beam model
k_beam_model = beam_model.k_scalar
k_scaling = 0.0000001

# stiffness value from beam like elements
k_beam = k_beam_model * k_scaling
print("K beam: ", k_beam)
# Create object of MDoFMixedModel
mixed_model = MDoFMixedModel(rho, area, target_freq, target_mode, zeta, floor_height, floor_num, k_beam, name)
...
static_force = mixed_model._load_distribution(floor_height, floor_num, wind_force)
static_analysis = StaticAnalysis(mixed_model)
...
dynamic_analysis = DynamicAnalysis(mixed_model, np.array([u0,v0,a0]), force_dynamic, [start_time,end_time], 
                   dt, "GenAlpha" ) 
