...
class ESWL(object):
    ...
    def __init__(self, structure_model, eigenvalue_analysis, response, load_signals, load_directions = ['x', 'y', 'z', 'a', 'b', 'g']): # not sure if this can be done with a list 
        ...
        self.eswl_total = {} # response in each direction
    ...
    def calculate_total_ESWL(self):
        ...
        for direction in self.load_directions:
            ...
            for i in range(1,len(mean_load)):
                mean_load[i] = np.mean(self.load_signals[direction][i])
            
            R_max = self._get_maximum_response(self.response)

            #TODO: check if the multiplication with peak factor hear is correct-> must this be done twice: still not sure
    ...
    def plot_eswl_load_components(self, components, response_label):
