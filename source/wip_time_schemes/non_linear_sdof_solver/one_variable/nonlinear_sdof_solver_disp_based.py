...
from math import *
from sympy import *
from time_schemes import euler_disp_based, euler_vel_based, bdf1_disp_based, bdf1_vel_based, bdf2_disp_based, \
    bdf2_vel_based


class SDoF:

    def __init__(self, time_scheme=None, numerical_scheme=None, K=1.0, M=1.0, C=0.2, f=None, u0=1.0, v0=0.0, a0=0.0,
                 dt=0.01):
    ...
    def calculate_residual_disp_based(self, time_scheme, t, u_n1, u_n, u_nm1=None, u_nm2=None, u_nm3=None):
        ...
        if time_scheme == 'bdf1':
            if self.numerical_scheme == 'Newton Raphson':
                ru = -C * (-u_n + u_n1) / dt - M * ((-u_n + u_n1) / dt - (u_n - u_nm1) / dt) / dt + f - u_n1 * self.K(
                    u_n1)
        ...
        if time_scheme == 'bdf2':
            if self.numerical_scheme == 'Newton Raphson':
                ru = -C * (-2.0 * u_n / dt + 1.5 * u_n1 / dt + 0.5 * u_nm1 / dt) - M * (
                            1.5 * (-2.0 * u_n / dt + 1.5 * u_n1 / dt + 0.5 * u_nm1 / dt) / dt - 2.0 * (
                                1.5 * u_n / dt - 2.0 * u_nm1 / dt + 0.5 * u_nm2 / dt) / dt + 0.5 * (
                                        1.5 * u_nm1 / dt - 2.0 * u_nm2 / dt + 0.5 * u_nm3 / dt) / dt) + f - u_n1 * self.K(
                    u_n1)
            elif self.numerical_scheme == 'Picard':
                ru = -C * (-2.0 * u_n / dt + 1.5 * u_n1 / dt + 0.5 * u_nm1 / dt) - M * (
                            1.5 * (-2.0 * u_n / dt + 1.5 * u_n1 / dt + 0.5 * u_nm1 / dt) / dt - 2.0 * (
