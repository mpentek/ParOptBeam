...
def integrate_spectra(y,x,digits=3):
    ...
    '''
    return [np.around(np.trapz(np.abs(y),x),digits), np.around(simps(abs(y),x),digits)] 
...
def cross_spectral_density(x,y, time_series, subtract_mean, use_fs = True, use_nfft = True, window_type = 'box'):
    '''
...
def plot_csd(csd_result, signal_labels, show_plot):
    f_j = 0.2
    plt.figure(3)
    plt.suptitle('CSD of ' + signal_labels[0] + ' & ' + signal_labels[1])
    #plt.semilogy(csd_result[0], np.real(csd_result[1]), label = 'csd')#, label = 'csd at f_j: ' + str(csd_f_j))
    plt.loglog(csd_result[0], np.abs(csd_result[1]), label = 'csd')#, label = 'csd at f_j: ' + str(csd_f_j))
...
window_type_avail = ['hann','box']

use_fs = True
use_nfft = True
nperseg = nperseg_avail[1]
window_type = window_type_avail[1]
...
#     s2 = s2 - np.mean(s2)
# PSD results
spectra_results = get_velocity_spectra_methods(time_series, s1, z=z_ref, z0=z0_ref, lux=lux_ref, subtract_mean=subtract_mean)
# CSD result
f_csd, csd_raw, csd_fj = cross_spectral_density(s1, s2, time_series,
                                             subtract_mean = subtract_mean,
                                             use_fs = True, use_nfft = True, window_type = 'box')
'''
...
if subtract_mean:
    ...
    sig_label = 'RMS'
# https://dsp.stackexchange.com/questions/11185/why-the-rms-of-a-psd-curve-is-the-root-of-the-area-below
#print(sig_label + ' psd (as integration with trapz and simps): ')
for case in ['w']:
    # give rms_spectra y = the psd of the signal, x = the corresposing freqencys of the signal 
    print('integrated psd_w(s1): ' , rms_spectra(spectra_results['psd_'+ case +''],spectra_results['n_'+ case +''])[0] )
    #print('   psd_'+ case +': ' + ", ".join([str(val) for val in rms_spectra(spectra_results['psd_'+ case +''],spectra_results['n_'+ case +''])]))
    # print('      this is ' + ", ".join([str(round(val/sig*100,2)) +\
    #     '%' for val in rms_spectra(spectra_results['psd_'+ case +''],spectra_results['n_'+ case +''])]) + ' of time series ' + sig_label)
...
if selected_signal_1 == selected_signal_2:
    ...
    print ('integrated csd(s1,s1):', round(abs(rms_csd[0]),digits))
  
# Consider content in ranges:
f_sample = 1/(series_t[1] - series_t[0])

# printing the rms of the time series and the rms using the integration of the PSD
if partial_freqs:
    freqs_1 = [0.025,0.25,2.5,5,10,25]
    freqs_2 = [2.5,5,10,25]
    for sel_freq in freqs_2:
        idx = sel_idx(spectra_results['n_w'],sel_freq)
        if not idx:
            continue
        print('\nFor frequency content up to ' + str(sel_freq)+ ' Hz: ')
        print('STD ', str(rms(butter_lowpass_filter(s1 - signal_mean, sel_freq, f_sample))))
        print('RMS ', str(rms(butter_lowpass_filter(s1, sel_freq, f_sample))))
        print(sig_label + ' psd content (as integration with trapz and simps): ')
        for case in ['w']:
        
            print('   psd_'+ case +': ' + ", ".join([str(val) for val in rms_spectra(spectra_results['psd_'+ case +''][:idx],spectra_results['n_'+ case +''][:idx])]))
            print('   this is ' + ", ".join([str(round(val/sig*100,2)) +\
                '%' for val in rms_spectra(spectra_results['psd_'+ case +''],spectra_results['n_'+ case +''])]) + ' of time series ' + sig_label)
        

#plot_result_spectra(spectra_results, freq_cutoff_ref, z_ref, signal_mean, selected_signal_1,show_plot = show_plots)
#plot_csd((f_csd, csd_raw), [selected_signal_1, selected_signal_2], show_plot = show_plots)
# round digits
...
cov_xy_csd_sqrt = rms_spectra(csd_raw, f_csd)[0]
cov_xy_csd_no_sqrt = integrate_spectra(csd_raw, f_csd)[0]
...
print('...csd cov sqrt:   ', round(abs(cov_xy_csd_sqrt),digits))#, round(abs(cov_xy_csd_sqrt[1]),digits))
print('...csd cov NO sqrt:', round(abs(cov_xy_csd_no_sqrt),digits))#, round(abs(cov_xy_csd_no_sqrt[1]),digits))
print('with no sqrt this is', round(cov_xy_np/cov_xy_csd_no_sqrt * 100,digits), '%', 'of the numpy cov (using trapz)')
...
cor_cov_csd_std = cov_xy_csd_no_sqrt /(std_s1 * std_s2)
...
print('\nthe real csd at f_j is', np.abs(csd_fj))
