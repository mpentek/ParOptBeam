...
considered_cases = [1, 2, 3, 15, 30, 60]
ramp_up_time = 30 * 1.5

# result directory 
if not os.path.isdir('output/Caarc/consolidated_plots'):
    os.mkdir('output/Caarc/consolidated_plots')
if not os.path.isdir('output/Caarc/consolidated_plots/spatial'):
    os.mkdir('output/Caarc/consolidated_plots/spatial')
output_dir = 'output/Caarc/consolidated_plots/spatial'
...
for key, value in case_suf.items():
    ...
    fig_max_dis_mag.suptitle('Maximum absolute top floor displacement with spatial reduction')
    n_row = len(case_mid)
    n_col = len(value)
    gs = gridspec.GridSpec(n_row, n_col)
    ...
    fig_rms_dis_mag.suptitle('RMS of top floor displacement with spatial reduction')
    n_row = len(case_mid)
    n_col = len(value)
    gs = gridspec.GridSpec(n_row, n_col)
    ...
    fig_max_acc_mag.suptitle('Maximum absolute top floor acceleration with spatial reduction')
    n_row = len(case_mid)
    n_col = len(value)
    gs = gridspec.GridSpec(n_row, n_col)
    ...
    fig_rms_acc_mag.suptitle('RMS of top floor acceleration with spatial reduction')
    n_row = len(case_mid)
    n_col = len(value)
    gs = gridspec.GridSpec(n_row, n_col)
    ...
    for i_damping_ratio, damping_ratio in enumerate(['0.000', '0.01', '0.025', '0.05']):
        ...
        for i in range(n_row):
            for j in range(n_col):
                ...
                xticks_labels = [str(val) for val in considered_cases]
                xticks_values = list(range(1, len(considered_cases) + 1))
                ax_kin_en[j][i].set_title(subplot_title_i)
                ax_kin_en[j][i].plot(xticks_values, [x/kin_en[-1] for x in kin_en],label=damping_ratio,
                ...
                #ax_kin_en[j][i].yaxis.set_major_formatter(FormatStrFormatter('%.4f'))    
                if i == n_row-1 : 
                ...
                ax_dis_max[j][i].plot(xticks_values, [x/max_val['disp_magn'][-1] for x in max_val['disp_magn']],label=damping_ratio,
                    color=LINE_TYPE_SETUP["color"][i_damping_ratio],
                    linestyle=LINE_TYPE_SETUP["linestyle"][i_damping_ratio],
                    marker=LINE_TYPE_SETUP["marker"][i_damping_ratio],
                    markeredgecolor=LINE_TYPE_SETUP["markeredgecolor"][i_damping_ratio],
                ...
                # ax_dis_max[j][i].yaxis.set_major_formatter(FormatStrFormatter('%.4f'))    
                if i == n_row-1 : 
                ...
                ax_dis_rms[j][i].plot(xticks_values, [x/rms_val['disp_magn'][-1] for x in rms_val['disp_magn']],label=damping_ratio,
                    color=LINE_TYPE_SETUP["color"][i_damping_ratio],
                    linestyle=LINE_TYPE_SETUP["linestyle"][i_damping_ratio],
                    marker=LINE_TYPE_SETUP["marker"][i_damping_ratio],
                    markeredgecolor=LINE_TYPE_SETUP["markeredgecolor"][i_damping_ratio],
                ...
                # ax_dis_rms[j][i].yaxis.set_major_formatter(FormatStrFormatter('%.4f'))    
                if i == n_row-1 : 
                ...
                ax_acc_max[j][i].plot(xticks_values, [x/max_val['acc_magn'][-1] for x in max_val['acc_magn']],label=damping_ratio,
                    color=LINE_TYPE_SETUP["color"][i_damping_ratio],
                    linestyle=LINE_TYPE_SETUP["linestyle"][i_damping_ratio],
                    marker=LINE_TYPE_SETUP["marker"][i_damping_ratio],
                    markeredgecolor=LINE_TYPE_SETUP["markeredgecolor"][i_damping_ratio],
                ...
                # ax_acc_max[j][i].yaxis.set_major_formatter(FormatStrFormatter('%.4f'))    
                if i == n_row-1 : 
                ...
                ax_acc_rms[j][i].plot(xticks_values, [x/rms_val['acc_magn'][-1] for x in rms_val['acc_magn']],label=damping_ratio,
                    color=LINE_TYPE_SETUP["color"][i_damping_ratio],
                    linestyle=LINE_TYPE_SETUP["linestyle"][i_damping_ratio],
                    marker=LINE_TYPE_SETUP["marker"][i_damping_ratio],
                    markeredgecolor=LINE_TYPE_SETUP["markeredgecolor"][i_damping_ratio],
                ...
                # ax_acc_rms[j][i].yaxis.set_major_formatter(FormatStrFormatter('%.4f'))    
                if i == n_row-1 : 
