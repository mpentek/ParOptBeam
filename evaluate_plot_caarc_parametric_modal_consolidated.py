...
considered_cases = [3, 6, 12, 18, 90, 180]
cases_not_to_plot = [90,180]
...
if not os.path.isdir('output/Caarc/consolidated_plots/modal'):
    os.mkdir('output/Caarc/consolidated_plots/modal')
output_dir = 'output/Caarc/consolidated_plots/modal'

for key, value in case_suf.items():
    # kinetic energy
    fig_kin_energy = plt.figure()
    fig_kin_energy.suptitle('Kinetic energy with modal reduction')
    n_row = len(case_mid)
    n_col = len(value)
    ...
    fig_max_dis_mag.suptitle('Maximum top floor displacement(magnitude) with modal reduction')
    n_row = len(case_mid)
    n_col = len(value)
    gs = gridspec.GridSpec(n_row, n_col)
    ...
    fig_rms_dis_mag.suptitle('RMS of top floor displacement(magnitude) with modal reduction')
    n_row = len(case_mid)
    n_col = len(value)
    gs = gridspec.GridSpec(n_row, n_col)
    ...
    fig_max_acc_mag.suptitle('Maximum top floor acceleration(magnitude) with modal reduction')
    n_row = len(case_mid)
    n_col = len(value)
    gs = gridspec.GridSpec(n_row, n_col)
    ...
    fig_rms_acc_mag.suptitle('RMS of top floor acceleration(magnitude) with modal reduction')
    n_row = len(case_mid)
    n_col = len(value)
    gs = gridspec.GridSpec(n_row, n_col)
    ...
    for i_damping_ratio, damping_ratio in enumerate(['0.000', '0.01', '0.025', '0.05']):
        ...
        for i in range(n_row):
            for j in range(n_col):
                    ...
                    for ii in range(len(time_series)):
                        if ii == 0:
                            ...
                            if (acc_x[ii]**2 + acc_y[ii]**2)**0.5 > val:
                                acc_max = [acc_x[ii], acc_y[ii], time_series[ii]]
                                val = (acc_x[ii]**2 + acc_y[ii]**2)**0.5
                    # calculate thr rms of acceleration 
                ...
                # plotting the kinetic energy values 
                xticks_labels = [str(val) for val in considered_cases][:-no_no_plot]
                xticks_values = list(range(1, len(considered_cases) + 1))[:-no_no_plot]
                ax_kin_en[j][i].set_title(subplot_title_i)
                
                ax_kin_en[j][i].plot(xticks_values, [x/kin_en[-1] for x in kin_en][:-no_no_plot],label=damping_ratio,
                ...
                ax_kin_en[j][i].set_xticks(xticks_values)
                if i == n_row-1 : 
                    ax_kin_en[j][i].set_xticklabels(xticks_labels)
                    ax_kin_en[j][i].set_xlabel("No. of modes")
                else: 
                ...
                ax_dis_max[j][i].plot(xticks_values, [x/max_val['disp_magn'][-1] for x in max_val['disp_magn']][:-no_no_plot],label=damping_ratio,
                    color=LINE_TYPE_SETUP["color"][i_damping_ratio],
                    linestyle=LINE_TYPE_SETUP["linestyle"][i_damping_ratio],
                    marker=LINE_TYPE_SETUP["marker"][i_damping_ratio],
                    markeredgecolor=LINE_TYPE_SETUP["markeredgecolor"][i_damping_ratio],
                ...
                ax_dis_max[j][i].set_xticks(xticks_values)
                if i == n_row-1 : 
                    ax_dis_max[j][i].set_xticklabels(xticks_labels)
                    ax_dis_max[j][i].set_xlabel("No. of modes")
                else: 
                ...
                ax_dis_rms[j][i].plot(xticks_values, [x/rms_val['disp_magn'][-1] for x in rms_val['disp_magn']][:-no_no_plot],label=damping_ratio,
                    color=LINE_TYPE_SETUP["color"][i_damping_ratio],
                    linestyle=LINE_TYPE_SETUP["linestyle"][i_damping_ratio],
                    marker=LINE_TYPE_SETUP["marker"][i_damping_ratio],
                    markeredgecolor=LINE_TYPE_SETUP["markeredgecolor"][i_damping_ratio],
                ...
                ax_dis_rms[j][i].set_xticks(xticks_values)
                if i == n_row-1 : 
                    ax_dis_rms[j][i].set_xticklabels(xticks_labels)
                    ax_dis_rms[j][i].set_xlabel("No. of modes")
                else: 
                ...
                ax_acc_max[j][i].plot(xticks_values, [x/max_val['acc_magn'][-1] for x in max_val['acc_magn']][:-no_no_plot],label=damping_ratio,
                    color=LINE_TYPE_SETUP["color"][i_damping_ratio],
                    linestyle=LINE_TYPE_SETUP["linestyle"][i_damping_ratio],
                    marker=LINE_TYPE_SETUP["marker"][i_damping_ratio],
                    markeredgecolor=LINE_TYPE_SETUP["markeredgecolor"][i_damping_ratio],
                ...
                ax_acc_max[j][i].set_xticks(xticks_values)
                if i == n_row-1 : 
                    ax_acc_max[j][i].set_xticklabels(xticks_labels)
                    ax_acc_max[j][i].set_xlabel("No. of modes")
                else: 
                ...
                ax_acc_rms[j][i].plot(xticks_values, [x/rms_val['acc_magn'][-1] for x in rms_val['acc_magn']][:-no_no_plot],label=damping_ratio,
                    color=LINE_TYPE_SETUP["color"][i_damping_ratio],
                    linestyle=LINE_TYPE_SETUP["linestyle"][i_damping_ratio],
                    marker=LINE_TYPE_SETUP["marker"][i_damping_ratio],
                    markeredgecolor=LINE_TYPE_SETUP["markeredgecolor"][i_damping_ratio],
                ...
                ax_acc_rms[j][i].set_xticks(xticks_values)
                if i == n_row-1 : 
                    ax_acc_rms[j][i].set_xticklabels(xticks_labels)
                    ax_acc_rms[j][i].set_xlabel("No. of modes")
                else: 
                    ax_acc_rms[j][i].set_xticklabels([])
                if i ==0 and j == n_col-1: 
                    ax_acc_rms[j][i].legend()
    fig_kin_energy.savefig(
        os_join(output_dir, 'kin_en_summary_consolidated_modal_'+str(key)+'.png'))
    plt.close(fig=fig_kin_energy)
    print('kin_en_summary_consolidated_modal_'+str(key)+'.png'+ 'file created')

    fig_max_dis_mag.savefig(
        os_join(output_dir, 'max_dis_magnitude_summary_consolidated_modal_'+str(key)+'.png'))
    plt.close(fig=fig_max_dis_mag)
    print('max_dis_magnitude_summary_consolidated_modal_'+str(key)+'.png'+ 'file created')

    fig_rms_dis_mag.savefig(
        os_join(output_dir, 'rms_dis_magnitude_summary_consolidated_modal_'+str(key)+'.png'))
    plt.close(fig=fig_rms_dis_mag)
    print('rms_dis_magnitude_summary_consolidated_modal_'+str(key)+'.png'+ 'file created')

    fig_max_acc_mag.savefig(
        os_join(output_dir, 'max_acc_magnitude_summary_consolidated_modal_'+str(key)+'.png'))
    print('max_acc_magnitude_summary_consolidated_modal_'+str(key)+'.png'+ 'file created')
    
    plt.close(fig=fig_max_acc_mag)
    fig_rms_acc_mag.savefig(
        os_join(output_dir, 'rms_acc_magnitude_summary_consolidated_modal_'+str(key)+'.png'))
    print('rms_acc_magnitude_summary_consolidated_modal_'+str(key)+'.png'+ 'file created')
    plt.close(fig=fig_rms_acc_mag)



print('plotting Caarc parametric modal results consolidated finished!')
