...
# considered_cases = [[6, 12, 18],
#                     [90, 180, 360]]

# MP: NOTE: adding 3
considered_cases = [[3, 6, 12],
                    [18, 90, 180]]
...
for key, value in case_suf.items():
    for damping_ratio in ['0.000', '0.01', '0.025', '0.05']:
        ...
        fig_kinematic = plt.figure()
        fig_kinematic.suptitle('Kinetic energy - modal red')
        n_row = len(case_mid)
        n_col = len(value)
        gs = gridspec.GridSpec(n_row, n_col)
        ax_kin_en = [[fig_kinematic.add_subplot(gs[i, j]) for i in range(n_row)]
                    for j in range(n_col)]
        ...
        for i in range(n_row):
            for j in range(n_col):
                ...
                ax_kin_en[j][i].set_title(file_name)
                ax_kin_en[j][i].plot(xticks_values, [x/kin_en[-1] for x in kin_en], 'ob--')
                ax_kin_en[j][i].set_xticklabels(xticks_labels)
                ax_kin_en[j][i].set_xticks(xticks_values)
        
        fig_kinematic.savefig(
            os_join(working_folder, 'kin_en_summary.png'))
        ...
        print(os_join(working_folder, 'kin_en_summary.png') + ' file created')

print('Evaluate Caarc parametric modal results finished!')